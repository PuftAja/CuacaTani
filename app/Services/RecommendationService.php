 ...
 *
 * Input:  array $prakiraan (dari WeatherService) + string $komoditas
 * Output: array rekomendasi per hari, format:
 * [
 *     ['tanggal' => '2026-06-21', 'rekomendasi' => '...', 'aksi' => 'siram|tunda_pemupukan|...'],
 *     ...
 * ]
 */
...
{
    /**
     * Threshold suhu (°C) — bisa disesuaikan per komoditas.
     */
    ...
    /**
     * Kurang dari ini dianggap turun hujan (mm/hari).
     */
     ...
     *
     * @param  array  $prakiraan  Data prakiraan dari WeatherService::getForecast()
     * @param  string $komoditas  'padi' | 'jagung'
     * @return array              Array of ['tanggal', 'rekomendasi', 'aksi']
     */
    public function getRekomendasi(array $prakiraan, string $komoditas): array
    {
        ...
        foreach ($prakiraan as $idx => $hari) {
            ...
            $rekomendasi = $this->tentukanRekomendasi($suhuMax, $suhuMin, $kondisi, $komoditas);
     ...
     */
    protected function tentukanRekomendasi(float $suhuMax, float $suhuMin, string $kondisi, string $komoditas): array
    {
        $th = $this->threshold[$komoditas] ?? $this->threshold['padi'];

        // ── 1. Hujan lebat → tunda pemupukan ──
        if (str_contains($kondisi, 'hujan') || str_contains($kondisi, 'petir')) {
            return [
                'pesan' => 'Hujan diprediksi hari ini. Tunda pemupukan agar pupuk tidak larut dan terbawa air hujan.',
        ...
        if (str_contains($kondisi, 'cerah') || str_contains($kondisi, 'berawan')) {
            return [
                'pesan' => 'Kondisi cuaca ideal. Waktu yang tepat untuk pemupukan dan perawatan rutin.',
        ...
        return [
            'pesan' => 'Tidak ada aksi khusus yang diperlukan. Pantau kondisi lahan secara berkala.',
            'aksi'  => 'tidak_ada',
        ];
    }
